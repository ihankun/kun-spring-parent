package com.ihankun.core.mq.producer.impl;
...
import com.ihankun.core.log.constant.TraceLogConstant;
import com.ihankun.core.log.context.TraceLogContext;
import com.ihankun.core.log.enums.LogTypeEnum;
import com.ihankun.core.mq.MqAutoConfiguration;
...
import com.ihankun.core.mq.producer.KunMqSendCallback;
...
import com.ihankun.core.mq.rocketmq.producer.KunMqProducer;
import com.ihankun.core.mq.rocketmq.producer.strategy.impl.GgrayKunProducerSelectQueueStrategy;
import com.ihankun.core.mq.rocketmq.producer.strategy.impl.GrayKunProducerSelectQueueStrategy;
import com.ihankun.core.mq.rocketmq.producer.strategy.impl.ProdKunProducerSelectQueueStrategy;
...
public class RocketMqProducer implements MqProducer {
    ...
    public void init(MqProperties config) {
        ...
        if (Boolean.FALSE.toString().equalsIgnoreCase(grayMark)) {
            ...
            mqFaultStrategy = new ProdKunProducerSelectQueueStrategy(mqProperties.getGraySize());
        } else if (Boolean.TRUE.toString().equalsIgnoreCase(grayMark)) {
            // 灰度节点逻辑：发送消息到queue[0-graySize]
            mqFaultStrategy = new GrayKunProducerSelectQueueStrategy(mqProperties.getGraySize());
        } else {
            // 灰灰度节点逻辑，发送消息到queue[(n-graySize)-n]
            mqFaultStrategy = new GgrayKunProducerSelectQueueStrategy(mqProperties.getGraySize());
        }
        producer = new KunMqProducer(producerGroupName, mqFaultStrategy, null, grayMark);
