...
//            String msg = "没有可用数据源，请检查数据源是否配置正确，线程" + threadName + ",用户" + (loginUserInfo == null ? "null" : JSON.toJSONString(loginUserInfo)) + ",域名" + DomainContext.get();
...
//            log.info("DomainDynamicDataSourceStrategy.determineDataSource.fail.first,threadName={},datasource={}", threadName, dataSource == null ? "域名为空，且符合默认线程，取第一个返回为空" : dataSource.getName());
//            return dataSource;
//        }
//
//        //其他线程，返回错误
//        String msg = "没有找到可选范围内的数据库连接池，线程" + threadName + ",用户" + (loginUserInfo == null ? "null" : JSON.toJSONString(loginUserInfo)) + ",域名" + DomainContext.get();
//        throw new RuntimeException(msg);
//    }
//
//
...
//        String name;
//        if (itemDataSourceList.isEmpty()) {
//            name = DynamicDataSourceContextHolder.peek();
//            if (StringUtils.isEmpty(name)) {
//                name = "master";
//            }
//        } else {
//            name = itemDataSourceList.get(0).getName();
//        }
//
...
//            throw new KunDbException("无法找到匹配的数据源" + ",thread:" + Thread.currentThread().getName() + ",域名:" + domain + ",数据源:" + dsName);
//        }
//        return source;
//    }
//}
